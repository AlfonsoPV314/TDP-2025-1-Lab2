...
bool STP::verifyValidity(const vector<pair<int, int>>& Te, double totalWeight) const {
    ...
    if (Te.size() < g->getTn()) {
        cout << "[STP::verifyValidity] Error: El numero de aristas de la aproximacion (" << Te.size() << ") es menor que el numero de nodos terminales original (" << g->getTn() << ")." << endl;
        isValid = false;
    }
    ...
    for(int i = 0; i < Te.size(); ++i) {
        ...
        if (g->getEdge(edge.first, edge.second) == g->getINF()) {
            cout << "[STP::verifyValidity] Error: La arista " << edge.first + 1 << "-" << edge.second + 1 << " (de la aproximacion) no existe en el grafo original." << endl;
            isValid = false;
        }
    ...
    for(int i = 0; i < g->getV(); ++i) {
        if (g->getT()[i] && find_if(Te.begin(), Te.end(), [i](const pair<int, int>& edge) {
            ...
            cout << "[STP::verifyValidity] Error: El nodo terminal original " << i + 1 << " no está conectado al el árbol de Steiner de la aproximacion." << endl;
            isValid = false;
        }
    }
    if(!isValid) {
        cout << "[STP::verifyValidity] La aproximacion no es valida. :( " << endl;
        return false;
    }
